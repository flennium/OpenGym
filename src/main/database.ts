...
export class GymDatabase {
  ...
  `);
    ...
    this.db
      ...
      .run();
    const refreshedMemberColumns = this.db.pragma(
      "table_info(members)",
    ) as any[];
    ...
    const membershipColumns = this.db.pragma(
    ...
    this.db
      ...
      .run();
    const latestSettingColumns = this.db.pragma(
      "table_info(settings)",
    ) as any[];
    if (
    ...
    this.db
      ...
      .run();
    this.db.exec(`CREATE TABLE IF NOT EXISTS member_face_templates(
