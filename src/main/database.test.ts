...
describe("security and dates", () => {
  ...
  it("stores appearance preferences independently for each staff account", () => {
    const db = fresh();
    db.setup({ gymName: "Atlas Gym", ownerName: "Owner", pin: "123456", locale: "en-DZ", currency: "DZD", timezone: "Africa/Algiers" });
    const second = Number(db.db.prepare("INSERT INTO staff(name,role_id,pin_hash,created_at,updated_at) VALUES(?,2,?,datetime('now'),datetime('now'))").run("Admin", hashPin("654321")).lastInsertRowid);
    expect(db.staffPreferences(1)).toEqual({ theme: "Pulse", darkMode: true });
    expect(db.staffPreferences(second)).toEqual({ theme: "Pulse", darkMode: true });
    db.updateStaffPreferences(1, { theme: "Ocean", darkMode: false });
    expect(db.staffPreferences(1)).toEqual({ theme: "Ocean", darkMode: false });
    expect(db.staffPreferences(second)).toEqual({ theme: "Pulse", darkMode: true });
    expect(db.auditLog({ search: "", staffId: 1 }).rows).toEqual(expect.arrayContaining([
      expect.objectContaining({ entityType: "staff_preferences", action: "updated" }),
    ]));
    db.close();
  });
});
...
describe("database invariants", () => {
  it("enforces owner-defined onboarding documents and stores their bytes", () => {
    const db = fresh();
    db.setup({ gymName: "Atlas Gym", ownerName: "Owner", pin: "123456", locale: "en-DZ", currency: "DZD", timezone: "Africa/Algiers" });
    const requirementId = db.addDocumentRequirement("Medical fitness certificate", 1);
    const input: any = { firstName: "Documented", lastName: "Member", phone: "", email: "", dateOfBirth: "", gender: "", address: "", emergencyName: "", emergencyPhone: "", status: "active", notes: "", photoPath: null };
    expect(() => db.saveMember(input, undefined, 1)).toThrow(/Medical fitness certificate/);
    const documentPath = join(dirname(db.path), "certificate.pdf");
    writeFileSync(documentPath, Buffer.from("%PDF-1.4 test certificate"));
    const memberId = db.saveMember({ ...input, documents: [{ requirementId, path: documentPath }] }, undefined, 1);
    const stored = db.db.prepare("SELECT file_name,mime_type,length(file_blob) size FROM member_documents WHERE member_id=?").get(memberId) as any;
    expect(stored).toMatchObject({ file_name: "certificate.pdf", mime_type: "application/pdf" });
  ...
  it("creates setup once and prevents duplicate open visits", () => {
    ...
    const plan = db.savePlan({ name: "Monthly", description: "", durationMonths: 1, trainingHours: null, priceMinor: 1000 }, undefined, owner.id);
    db.assign(member, plan, new Date().toISOString().slice(0, 10), false, owner.id);
  ...
  it("ends a hybrid membership when either its date or training hours run out", () => {
    const db = fresh();
    db.setup({ gymName: "Atlas Gym", ownerName: "Owner", pin: "123456", locale: "en-DZ", currency: "DZD", timezone: "Africa/Algiers" });
    const member = db.saveMember({ firstName: "Timed", lastName: "Member", phone: "", email: "", dateOfBirth: "", gender: "", address: "", emergencyName: "", emergencyPhone: "", status: "active", notes: "", photoPath: null }, undefined, 1);
    const plan = db.savePlan({ name: "One month / one hour", description: "", durationMonths: 1, trainingHours: 1, priceMinor: 1000 }, undefined, 1);
    const membership = db.assign(member, plan, new Date().toISOString().slice(0, 10), false, 1);
    const visit = db.checkIn(member, 1);
    db.db.prepare("UPDATE attendance SET checked_in_at=? WHERE id=?").run(new Date(Date.now() - 61 * 60000).toISOString(), visit);
    db.checkOut(visit, 1);
    const exhausted = db.db.prepare("SELECT status,used_minutes FROM memberships WHERE id=?").get(membership) as any;
    ...
    expect(db.db.prepare("SELECT status,used_minutes FROM memberships WHERE id=?").get(membership)).toMatchObject({ status: "active", used_minutes: 0 });
    db.close();
  });
  it("supports an hours-only membership without a calendar expiry", () => {
    const db = fresh();
    db.setup({ gymName: "Atlas Gym", ownerName: "Owner", pin: "123456", locale: "en-DZ", currency: "DZD", timezone: "Africa/Algiers" });
    const member = db.saveMember({ firstName: "Hours", lastName: "Only", phone: "", email: "", dateOfBirth: "", gender: "", address: "", emergencyName: "", emergencyPhone: "", status: "active", notes: "", photoPath: null }, undefined, 1);
    const plan = db.savePlan({ name: "Ten training hours", description: "", durationMonths: 0, trainingHours: 10, priceMinor: 1000 }, undefined, 1);
    const membership = db.assign(member, plan, "2026-01-01", false, 1);
    expect(db.db.prepare("SELECT end_date,training_minutes_limit FROM memberships WHERE id=?").get(membership)).toMatchObject({ end_date: "9999-12-31", training_minutes_limit: 600 });
    expect(db.checkIn(member, 1)).toBeGreaterThan(0);
    expect((db.db.prepare("SELECT member_code FROM members WHERE id=?").get(member) as any).member_code).toMatch(/^\d{10}$/);
    db.close();
  });
  ...
  it("extends the exact expiry timestamp instead of rounding freeze time to days", () => {
    const db = fresh();
    db.setup({ gymName: "Atlas Gym", ownerName: "Owner", pin: "123456", locale: "en-DZ", currency: "DZD", timezone: "Africa/Algiers" });
    const member = db.saveMember({ firstName: "Exact", lastName: "Freeze", phone: "", email: "", dateOfBirth: "", gender: "", address: "", emergencyName: "", emergencyPhone: "", status: "active", notes: "", photoPath: null }, undefined, 1);
    const plan = db.savePlan({ name: "Exact month", description: "", durationMonths: 1, trainingHours: null, priceMinor: 1000 }, undefined, 1);
    const membership = db.assign(member, plan, "2026-09-01", false, 1);
    db.membershipAction(membership, "freeze", 1);
    ...
    db.db.prepare("UPDATE membership_freeze_periods SET frozen_at=? WHERE membership_id=? AND resumed_at IS NULL").run(frozenAt, membership);
    db.membershipAction(membership, "resume", 1);
    const row = db.db.prepare("SELECT expires_at FROM memberships WHERE id=?").get(membership) as any;
    const expected = new Date(new Date("2026-09-30T23:59:59.999Z").getTime() + 36 * 60 * 60 * 1000);
    expect(Math.abs(new Date(row.expires_at).getTime() - expected.getTime())).toBeLessThan(2000);
    const history = db.membershipFreezeHistory(membership) as any[];
    expect(Math.abs(history[0].duration_ms - 36 * 60 * 60 * 1000)).toBeLessThan(2000);
    db.close();
  });
  it("blocks banned members and issues sequential receipts", () => {
    ...
    db.updateSettings({ gymName: "Atlas Gym", locale: "en-DZ", currency: "DZD", timezone: "Africa/Algiers", address: "1 Atlas Way", phone: "0550", email: "hello@atlas.test", taxId: "TAX-1", receiptFooter: "Train strong.", receiptPaper: "A4", receiptColor: "#147D5D", logoPath: "C:/branding/logo.png", faceRecognitionEnabled: false }, 1);
    ...
    const snapshot = JSON.parse((db.db.prepare("SELECT gym_snapshot FROM payments ORDER BY id LIMIT 1").get() as any).gym_snapshot);
    expect(snapshot).toMatchObject({ name: "Atlas Gym", address: "1 Atlas Way", receiptFooter: "Train strong.", receiptColor: "#147D5D", logoPath: "C:/branding/logo.png" });
    const purchase = JSON.parse((db.db.prepare("SELECT member_snapshot FROM payments ORDER BY id LIMIT 1").get() as any).member_snapshot);
    expect(purchase).toMatchObject({ name: "A Member", planName: "Monthly", membershipStart: "2026-01-01", membershipEnd: "2026-01-31" });
    db.close();
  });
  it("seeds editable role permissions and produces reports", () => {
    ...
    expect(audit.rows).toEqual(expect.arrayContaining([expect.objectContaining({ staffName: "Owner", entityType: "permission", action: "updated" })]));
    expect(db.auditLog({ search: "", staffId: 1 }).rows.length).toBeGreaterThan(0);
    db.close();
  });
});
