...
export function inspectBackup(path: string) {
  ...
  try {
    ...
    if (integrity[0]?.quick_check !== "ok") throw new Error("Database integrity check failed");
    const version = (probe.prepare("SELECT MAX(version) version FROM schema_migrations").get() as { version?: number })?.version;
    if (!version || version > SUPPORTED_SCHEMA_VERSION) throw new Error("Unsupported database version");
    return { version };
  } finally {
    probe.close();
  }
